...
{
	printk(KERN_INFO "I2C Probe begin");
...
static int lcdi2c_remove(struct i2c_client *client)
{
	LcdData_t *data = i2c_get_clientdata(client);
        ...
        dev_info(&client->dev, "LCD going to be removed");
        if (data)
            lcdfinalize(data);
...
static const struct of_device_id lcdi2c_dts = {
		{.compatible = "shtl,lcdi2c-drv"},	{}
};
MODULE_DEVICE_TABLE(of, lcdi2c_dts);
...
static struct i2c_driver lcdi2c_driver = {
	.driver = {
		...
		.of_match_table = lcdi2c_dts,
};
MODULE_DEVICE_TABLE(of, talos_dt_ids); // Add our device to Devices Table - for "Hot Pluggin" //

static struct platform_driver pdrv = { // Platform Driver //
	.probe				= lcdi2c_pdev_probe,
	.remove 			= lcdi2c_pdev_remove,
	.driver = {
			.name		= DEVICE_NAME,
			.owner  	= THIS_MODULE,
			.of_match_table = lcdi2c_dts,
...
/*********** probe function *********/
static int lcdi2c_pdev_probe(struct platform_device *pdev) {

	printk(KERN_INFO "PDEV Probe begin");

	adapter = of_get_i2c_adapter_by_node(pdev->dev.of_node);

	client = of_i2c_register_device(adapter,pdev->dev.of_node);

    int ret = i2c_add_driver(&lcdi2c_driver);
    if (ret)
    {
        i2c_put_adapter(adapter);
        printk(KERN_INFO "Cant get I2C adapter");
        return ret;
    }
    i2c_put_adapter(adapter);
...
static int lcdi2c_pdev_remove(struct platform_device *pdev) {
